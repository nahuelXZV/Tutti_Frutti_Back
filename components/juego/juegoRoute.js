...
const juegoController = require('./juegoController');
const express = require('express');
const passport = require('passport');

const router = express.Router();
const controller = new juegoController();
...
router.get(
  '/:id',
  // passport.authenticate('jwt', { session: false }), // Middleware de autenticación
  validatorHandler(getJuegoSchema, 'params'), // Middleware de validación
  async (req, res, next) => {
...
router.post(
  '/',
  validatorHandler(addJuegoSchema, 'body'),
  async (req, res, next) => {
    try {
...
router.put(
  ...
  validatorHandler(getJuegoSchema, 'params'),
  validatorHandler(editJuegoSchema, 'body'),
  async (req, res, next) => {
    const { id } = req.params;
    const body = req.body; //used for getting the body
    await controller
